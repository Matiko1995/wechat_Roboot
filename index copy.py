...
def parse_order_message(content):
    ...
    # 尝试匹配没有逗号的格式：xxx xxx xxx 共xx人
    # 这种格式可能会误匹配，但根据你的示例，我们也需要支持
    no_comma_pattern = r'(.+?)\s+共(\d+)人'
    match = re.search(no_comma_pattern, content)
    if match:
        order_content = match.group(1).strip()  # 人员名单，去除首尾空格
        order_count = int(match.group(2))  # 人数
        print(f"成功解析订餐(无逗号格式): 内容={order_content}, 人数={order_count}")
        return order_content, order_count
    ...
    try:
        ...
        if os.path.exists(excel_path):
            ...
            try:
                ...
                print(f"成功读取现有数据，共{len(existing_data)}行")
                ...
                for order in orders:
                    ...
                    if key not in existing_keys:
                        ...
                        print(f"跳过已存在的订单: {key}")
                
                print(f"过滤后剩余 {len(filtered_orders)} 条新订单")
                ...
                with pd.ExcelWriter(excel_path, engine='openpyxl', mode='a') as writer:
                    ...
                    print(f"更新订单数据到 {excel_path} 的 {today} 表格，共{len(combined_df)}行")
                
            except Exception as e:
                print(f"读取或更新现有Excel文件时出错: {e}")
                # 如果读取失败，尝试使用临时文件
                base, ext = os.path.splitext(excel_path)
                temp_path = f"{base}_temp_{int(time.time())}{ext}"
                
                # 创建新文件
                df = pd.DataFrame(orders)
                df.to_excel(temp_path, sheet_name=today, index=False)
                print(f"创建临时Excel文件: {temp_path}")
        else:
            # 创建新文件
            df = pd.DataFrame(orders)
        ...
        try:
            ...
            simple_path = os.path.join(SAVE_DIR, f"订餐记录_{timestamp}.xlsx")
            
            # 保存数据
            df = pd.DataFrame(orders)
            df.to_excel(simple_path, index=False)
            print(f"使用简单方式保存到: {simple_path}")
...
def generate_summary(orders, group_name, from_excel=False):
    ...
    Args:
        orders: 订单列表
        group_name: 群聊名称
        from_excel: 是否从Excel中读取完整数据
    """
    if not orders and not from_excel:
        return f"{group_name}今日暂无订餐记录"
    
    # 获取今天的日期
    today = datetime.now().strftime("%Y年%m月%d日")
    today_date = get_today_date()  # 获取YYYY-MM-DD格式的日期
    ...
    if from_excel:
        try:
            ...
            if os.path.exists(excel_path):
                # 读取Excel中当天的所有订单
                excel_orders = pd.read_excel(excel_path, sheet_name=today_date, engine='openpyxl')
...
def monitor_group():
    ...
    while True:
        try:
            ...
            # 每隔一段时间打印一次心跳信息
            if current_time - last_check_time > 60:  # 每分钟打印一次
            ...
            for group_name, current_msgs in current_msgs_all.items():
                if not current_msgs:
                    print(f"{group_name} 没有获取到消息，等待10秒后重试")
                    time.sleep(10)
                    continue
